...
st.set_page_config(
    page_title="Dự đoán nguy cơ bệnh tim",
    page_icon="❤️",
    layout="wide"
)
...
model = RandomForestClassifier(n_estimators=180, random_state=42, max_depth=6)
model.fit(X_train, y_train)

y_pred = model.predict(X_test)
accuracy = accuracy_score(y_test, y_pred)
f1 = f1_score(y_test, y_pred, pos_label="Nguy cơ cao")
...
st.markdown("""
<div class="hero">
    <h1>Dự đoán nguy cơ bệnh tim bằng Machine Learning ❤️</h1>
    <p>
    Ứng dụng Random Forest để phân tích các chỉ số sức khỏe như tuổi, huyết áp,
    cholesterol, nhịp tim, đường huyết và thói quen vận động nhằm dự đoán nguy cơ bệnh tim.
    </p>
    <span class="badge">Y tế</span>
    <span class="badge">Python</span>
    <span class="badge">Streamlit</span>
    <span class="badge">Random Forest</span>
    <span class="badge">Dữ liệu CSV</span>
</div>
...
metrics = [
    ...
    (f"{accuracy*100:.1f}%", "Độ chính xác", "✅"),
...
st.markdown('<div class="section-title">2. Nhập dữ liệu để dự đoán</div>', unsafe_allow_html=True)
...
with result_col:
    ...
    if predict_btn:
        ...
        proba = model.predict_proba(input_df).max() * 100
        ...
        <div class="notice">
            <b>Hướng dẫn:</b><br>
            Nhập các chỉ số sức khỏe ở khung bên trái, sau đó bấm nút dự đoán.
        </div>
        """, unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)

st.markdown('<div class="section-title">3. Trực quan hóa dữ liệu</div>', unsafe_allow_html=True)
c1, c2 = st.columns(2)
...
st.markdown('<div class="section-title">4. Đánh giá mô hình Random Forest</div>', unsafe_allow_html=True)
model_col, import_col = st.columns(2)
...
st.markdown('<div class="section-title">5. Dữ liệu thực nghiệm</div>', unsafe_allow_html=True)
st.markdown('<div class="glass-card">', unsafe_allow_html=True)
st.write("Bảng dữ liệu mẫu được lưu trong file `heart_health_data.csv` và dùng để huấn luyện mô hình.")
